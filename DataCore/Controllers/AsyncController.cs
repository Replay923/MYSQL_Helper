...
namespace DataCore.Controllers
{
    ...
    public class AsyncController : Controller
    {
        ...
        [HttpGet("/ReplayConector/NewsDetail")]
        public async Task<IActionResult> GetNewsDetail()
        {
            using (var db = new AppDb())
            {
                await db.Connection.OpenAsync();
                PostConector postConector = new PostConector(db);
                postConector.operationName = "NewsDetailPost";
                postConector.operateType = "SelectAll";
                var result = await postConector.ExcuteAsync();
                return new OkObjectResult(result);
            }
        }
        [HttpGet("/ReplayConector/NewsItem")]
        public async Task<IActionResult> GetNewsItem()
        {
            using (var db = new AppDb())
            {
                await db.Connection.OpenAsync();
                PostConector postConector = new PostConector(db);
                postConector.operationName = "NewsItemPost";
                postConector.operateType = "SelectAll";
                var result = await postConector.ExcuteAsync();
                return new OkObjectResult(result);
            }
        ...
        // POST api/async
        [HttpPost("/ReplayConector/dev")]
        public async Task<IActionResult> DevPost([FromBody]PostConector body)
        {
            using (var db = new AppDb())
            {
                await db.Connection.OpenAsync();
                body.Db = db;
                var result = await body.DevExcuteAsync();
                return new OkObjectResult(result);
            }
        }
        // POST api/async
        [HttpPost("/ReplayConector/guest")]
        public async Task<IActionResult> Post([FromBody]PostConector body)
        {
            using (var db = new AppDb())
            {
                await db.Connection.OpenAsync();
                body.Db = db;
                var result = await body.ExcuteAsync();
                return new OkObjectResult(result);
            }
        }
